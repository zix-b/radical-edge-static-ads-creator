...
export async function POST(request: Request) {
  ...
  const content: Array<Record<string, string>> = [{
    ...
    text: [
      ...
      "For anonymous proof, do not lead with the client name. Use the client type, market context, concrete problem, and result.",
      "Strong anonymised proof should still show what happened, why it mattered, and what bottleneck was removed.",
      "If the proof is a WhatsApp, email, DM, or sales message, treat the exact buyer moment as the hook. Do not summarize it into bland marketing language.",
      "Good headline pattern: [specific result] from [specific business mechanism], not [wrong solution they were trying].",
      "Good context pattern: A [client type] was stuck with [business bottleneck]. The proof shows [specific outcome].",
      "Use the internal proof-ad reference library logic: client type, business bottleneck, wrong solution, buyer moment, mechanism, result, redaction risk.",
      "The best output gives the ad generator enough metadata to write a concrete proof ad without showing the client's name.",
      ...
      JSON.stringify({
        ...
        headlineOptions: ["3-8 word image headline, result or buyer moment first", "3-8 word image headline, no client name"],
