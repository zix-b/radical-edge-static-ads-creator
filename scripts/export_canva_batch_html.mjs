...
function buildHeadline(source) {
  ...
  if (source.sourceType === "Conversion") {
    if (/sales training|contextualised sales|personal brand/i.test(proof)) {
      return {
...
function proofBlock(source, id) {
  if (source.sourceType === "Conversion" && /proof screenshot/i.test(source.fileType ?? "")) {
    ...
    return `<div class="proof conversion-shot">${image}<span>SCREENSHOT PROOF</span><strong>Place the approved Conversion screenshot in this frame</strong><span>Blur names, numbers, profile pictures and sensitive details</span></div>`;
  }

  return `<div class="proof transcript-signal"><span>COPY-ONLY PROOF</span><strong>Private proof shaped the headline and claim</strong><span>No client name, photo or private transcript text shown</span></div>`;
}
