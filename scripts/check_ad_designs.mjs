...
assert.ok(homePageSource.includes("currentCanvaDesignUrl"), "Homepage must expose the current Canva design link.");
assert.ok(homePageSource.includes("https://www.canva.com/d/J_Xe7aIYwzlPcaz"), "Homepage must link to the current editable Canva design.");
assert.ok(homePageSource.includes("Open Canva design"), "Homepage CTA must open the Canva design.");
assert.ok(!homePageSource.includes("Open generated proof cards"), "Homepage must not send users to generated preview cards.");
assert.ok(!homePageSource.includes("Proof preview set"), "Homepage must not label the output as a preview set.");
assert.ok(!homePageSource.includes("Generate proof previews"), "Homepage must not expose the old preview generator.");
